...
def graficar_multiples_corridas(capitales, capital_inicial):
    for i in capitales:
        plt.plot(i)
    plt.xlabel('Numero de Apuesta')
    plt.ylabel('Capital')
...
def graficar_promedio_estrategia(promedio_estrategia, capital_inicial):
    plt.title("Promedio de capital estrategia X")
    plt.plot(promedio_estrategia)
    plt.xlabel('Numero de Apuesta')
    plt.ylabel('Capital')
    plt.axhline(y=capital_inicial, color="red", linestyle='--')
    plt.show()
...
def apostar(cant_corridas, apuesta_inicial, capital_inicial, cantidad_de_apuestas, estrategia, capital_ilimitado):
    #promedio_estrategia = np.zeros(cantidad_de_apuestas)
    ...
    capitales_transpuesta = [list(value) for value in zip(*capitales)]
    promedio_capital = [np.average(value) for value in capitales_transpuesta]
    frecuencias_transpuesta = [list(value) for value in zip(*frecuencias)]
    promedio_frecuencia = [np.average(value) for value in frecuencias_transpuesta]
    #graficar_promedio_estrategia(promedio_capital, capital_inicial)
    #graficar_frec(promedio_frecuencia)
    graficar_multiples_corridas(capitales,capital_inicial)
    print('uwu')

        #for i in range(cantidad_de_apuestas):
        #    promedio_estrategia[i] += capital_historico[i]  # acumulo el capital en cada tirada
    ...
    #for i in range(cantidad_de_apuestas):
     #   promedio_estrategia[i] /= cant_corridas  # calculo el promedio del capital en cada tirada

    #if cant_corridas > 1:
    #    graficar_promedio_estrategia(promedio_estrategia, capital_inicial)
...
cant_tiradas = int(100)  # int(sys.argv[2])
cant_corridas = int(50)  # int(sys.argv[4])
# numero_elegido = int(sys.argv[6])
estrategia = "D"  # sys.argv[8].upper()
capital_ilimitado = True  # sys.argv[10] == 'i'
